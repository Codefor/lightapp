...
<body>
    <header id="header" style="display:none">
        <input type="button" class="home" style="display:none"/>
            ...
            <ul class="reply-list" id="reply-list">
                ...
                <li>
                    ...
                    <div class="list-footer">
                        ...
                        <a class="comment-reply" data-id="<?php echo $comment['id']?>" data-name="<? echo sprintf("%s:%s",$comment['name'],$comment['content']);?>">回复</a>
                    </div>
                    <ul class="reply-reply-list">
                        <?php foreach($comment['subitems'] as $reply){?>
                        <li>
                            <h6>
                                <span class="replier">@<?php echo $reply['name']?>:</span>
                                <a class="comment-reply" data-id="<?php echo $comment['id']?>" data-name="<? echo sprintf("%s:%s",$reply['name'],$reply['content']);?>">回复</a>
                                <span class="reply-time"><?php echo $reply['t']?></span>
                            </h6>
    ...
    <script>
        ...
        $(document).on('tap', '.comment-reply', function(e) {
            ...
            if($(e.target).closest('.reply-reply-list').length) {
                $replyModal.find('textarea').attr('placeholder', '//@'+replyTo);

                $targetComment = $targetComment.parent().closest('li');
            }else{
                $replyModal.find('textarea').attr('placeholder', '//@'+replyTo);
                //$replyModal.find('textarea').attr('placeholder', '说点什么吧');
            }
        });
